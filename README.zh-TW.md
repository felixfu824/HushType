...
**隱私與本地優先。** 語音永遠不離開你的 Mac——模型完全在本機執行，無雲端、無帳號、無遙測。只有首次一次性模型下載（約 675 MB），之後完全離線（事先下載即可完全離網）。

**記憶體友善——與你的 AI 助手共存。** 磁碟約 675 MB（不到 Whisper Large v3 Turbo 的一半），效能卻可媲美體積大三倍的模型——小到能在 8 GB 的 Mac 上與 Claude Code、Cursor 和瀏覽器同時運行。選單點一下即釋放約 2 GB，下次按住 Right ⌥ 自動重新載入。
...
**字幕兩種模式。** 本機 **Live Caption（即時字幕）** 用同一套裝置端管線把字幕顯示在浮動面板上：免費、離線、飛機上也能用。可選的 **Live Translated Caption（即時翻譯字幕）** 把音訊串到 OpenAI 的 `gpt-realtime-translate`，即時產生 14 種語言的外語字幕——金鑰是你的、音訊是你的、帳單是你的，絕不自動啟動。
