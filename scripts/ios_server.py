#!/usr/bin/env python3
"""
Lamitype iOS Server — proxy for mlx-audio with OpenCC s2twp conversion.
...
app = FastAPI(title="Lamitype iOS Server")
...
@app.get("/")
def root():
    return {"status": "ok", "service": "Lamitype iOS Server", "opencc": Path(OPENCC_PATH).exists()}
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Lamitype iOS Server")
    ...
    print(f"[ios_server] Lamitype iOS Server")
    ...
    # Create a new process group so the parent (Lamitype app) can kill us + children
    os.setpgrp()
