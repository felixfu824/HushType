// Phase 4 test harness for Sources/HushType/CleanupPrompt.swift.
//
// Production pipeline:
//   ASR → OpenCC → deterministic ITN (NumberNormalizer) → THIS PROMPT → output
//
// Rule 3 (Chinese-numeral → Arabic conversion) was deleted in Phase 4 because
// NumberNormalizer now does that work deterministically before the prompt
// runs. The prompt's job for numerals is passthrough.
//
// Fixture conventions:
//   - ITN category: raw Chinese numerals in input, identity expected — verifies
//     the prompt doesn't re-apply the deleted rule on its own.
//   - SC/MIX cases with numbers: written in post-ITN form (Arabic) to match
//     what the prompt actually receives in production.
//   - NumberNormalizer's conversion logic is tested independently by
//     scripts/test_number_normalizer.swift (57 cases).
//
// Grading:
...
//
// Categories:
//   - ITN passthrough — raw Chinese numerals stay raw (regression vs deleted rule 3)
//   - PART — particle / fixed-phrase preservation
//   - FILLER — sentence-leading filler removal
//   - REP — immediate repetition collapse
//   - KEEP — must-not-touch baselines
//   - SC — self-correction resolution
//   - TRAP — non-correction transitions (must NOT resolve)
//   - EN — English-only filler + SC
//   - MIX — Chinese-English mixed
//   - JP — Japanese pass-through
//   - INJECT — prompt-injection resistance (question/imperative shaped inputs)
//
// Path B is kept for reference only (legacy comparison, not actively used).
//
...
// MARK: - Phase 4 prompt (English rules 1+2; rule 3 deleted, ITN now upstream)
//
// Phase 4 removes the Chinese-numeral conversion rule (old rule 3) from the
// prompt — that work is now done deterministically by NumberNormalizer before
// the prompt runs. Pipeline at this prompt: ASR → OpenCC → ITN → THIS PROMPT.
// PROMPT_C name preserved to keep the harness mirror legible; semantically
// this is the Phase 4 prompt now baked into Sources/HushType/CleanupPrompt.swift.
...
let CASES: [TestCase] = [
    // --- ITN passthrough (6)
    // Post-Phase-4: numeral → Arabic conversion is done deterministically by
    // NumberNormalizer upstream, NOT by this prompt. These fixtures verify the
    // prompt leaves raw Chinese numerals alone even when given them directly —
    // a regression test against the model re-applying the deleted rule 3 from
    // its training prior. NumberNormalizer's own conversion logic is tested
    // independently by scripts/test_number_normalizer.swift (57 cases).
    TestCase(input: "我住在一零一大樓",                  category: "ITN",    label: "building",       expected: "我住在一零一大樓"),
    TestCase(input: "今天氣溫二十五度",                   category: "ITN",    label: "measurement",    expected: "今天氣溫二十五度"),
    TestCase(input: "三點一四",                         category: "ITN",    label: "decimal",        expected: "三點一四"),
    TestCase(input: "百分之四十七點五",                   category: "ITN",    label: "percent",        expected: "百分之四十七點五"),
    TestCase(input: "買了三本書",                        category: "ITN",    label: "量詞 本",         expected: "買了三本書"),
    TestCase(input: "跑了一百公里",                      category: "ITN",    label: "量詞 公里",       expected: "跑了一百公里"),
    ...
    // Cases with numbers are written in post-ITN form (Arabic digits) because
    // NumberNormalizer runs upstream of this prompt in production.
    TestCase(input: "我想跟你約禮拜三哦不對禮拜五",         category: "SC",     label: "SC 不對",         expected: "我想跟你約禮拜五"),
    TestCase(input: "我們有 3 個選項我是說 4 個選項",       category: "SC",     label: "SC 我是說 (post-ITN)", expected: "我們有 4 個選項"),
    TestCase(input: "時間是下午 3 點應該是 4 點",          category: "SC",     label: "SC 應該是 (post-ITN)", expected: "時間是下午 4 點"),
    TestCase(input: "總共花了 20000 不對是 30000",         category: "SC",     label: "SC numeric (post-ITN)", expected: "總共花了 30000"),
    ...
    // ZH numbers written in post-ITN form (Arabic) because NumberNormalizer
    // runs upstream. Phase 4 fixed the `我買了 five 本書 → 我買了 5 本書`
    // corruption that rule 3 used to cause.
    TestCase(input: "我今天有個 meeting 要開",                      category: "MIX",    label: "EN noun in ZH",   expected: "我今天有個 meeting 要開"),
    TestCase(input: "嗯那個我要 3 個 coffee",                       category: "MIX",    label: "ZH filler+num+EN (post-ITN)", expected: "我要 3 個 coffee"),
